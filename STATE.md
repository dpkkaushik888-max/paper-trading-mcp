...
| S25 | Strategy Discovery Loop ("Agent 1") | **IMPLEMENTED** (awaiting UAT) | Full propose→search→prove→promote pipeline built: `discovery/{primitives,candidate,generate,search,gates,registry_manifest}.py`, `loops/research_loop.py` (StrategyDiscoveryLoop), `loops/agent.propose_candidates`, `scripts/run_discovery.py`. Anti-overfitting enforced: trial budget + deflated-Sharpe (extreme-value/Bonferroni) + ≥2/3 sub-period robustness; holdout read once per survivor. Deterministic (seed→same candidates+verdicts). 47 discovery tests; full suite 198 green. Search is candidate-native on CryptoLeaf (not run_walk_forward — see spec note). UAT: run `scripts/run_discovery.py` on 5y data. |
